...
open class BuiltInClassesAreSerializableOnJvm(
    ...
    private fun getAdditionalFunctions(
        ...
        val platformVersions =
                SmartSet.create(
                        j2kClassMap.mapPlatformClass(javaAnalogueDescriptor).map { it.fqNameSafe })

        val isMutable = j2kClassMap.isMutable(classDescriptor)

        val jClass = (javaAnalogueDescriptor.source as? JavaSourceElement)?.javaElement as? JavaClass ?: return emptyList()

        val c = javaAnalogueDescriptor.c
        val scope =
                LazyJavaClassMemberScope(
                        c.replaceComponents(
                                c.components.replace(
                                        javaResolverCache = JavaResolverCache.EMPTY)),
                        javaAnalogueDescriptor, jClass, isUsedForBuiltInScopeCalculation = true)
    ...
    companion object {
        ...
        private val IGNORE_BY_DEFAULT_CLASS_FQ_NAMES = setOf(FqName("java.lang.String"))
