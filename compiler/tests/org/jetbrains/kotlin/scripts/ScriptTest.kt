...
class ScriptTest {
            ...
            suppressOutput: Boolean = false): Class<*>?
    {
        ...
        try {
            val configuration = KotlinTestUtils.compilerConfigurationForTests(ConfigurationKind.JDK_ONLY, TestJdkKind.FULL_JDK)
    ...
    private fun numIntParam(name: String = "num"): List<ScriptParameter> {
        return listOf(ScriptParameter(Name.identifier(name), JvmPlatform.builtIns.intType))
    }
}
