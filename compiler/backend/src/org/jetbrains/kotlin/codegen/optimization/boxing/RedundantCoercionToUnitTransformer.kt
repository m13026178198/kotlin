...
class RedundantCoercionToUnitTransformer : MethodTransformer() {
    ...
    private class Transformer(val methodNode: MethodNode) {
        ...
        private val resultMovedInsns = hashSetOf<AbstractInsnNode>()
        ...
        private fun computeResultMovedInsns() {
            for (i in 0..insns.lastIndex) {
                ...
                when (insn.opcode) {
                    Opcodes.DUP,
                    Opcodes.DUP_X1 -> {
                        val x = frame.top() ?: throwIllegalStackInsn(i)
                        recordResultMoved(x)
                    }
                    Opcodes.DUP2,
                    Opcodes.DUP2_X1,
                    Opcodes.DUP2_X2 -> {
                        ...
                        recordResultMoved(x)
                        if (x.size == 1) {
                            val y = frame.peek(1) ?: throwIllegalStackInsn(i)
                            if (y.size != 1) throwIllegalStackInsn(i)
                            recordResultMoved(y)
                        }
                    }
                    Opcodes.SWAP -> {
                        val x = frame.top() ?: throwIllegalStackInsn(i)
                        if (x.size != 1) throwIllegalStackInsn(i)
                        val y = frame.peek(1) ?: throwIllegalStackInsn(i)
                        if (y.size != 1) throwIllegalStackInsn(i)
                        recordResultMoved(x)
                        recordResultMoved(y)
        ...
        private fun recordResultMoved(sv: SourceValue) {
            resultMovedInsns.addAll(sv.insns)
        }
        ...
        private fun propagatePopBackwards(insn: AbstractInsnNode, poppedValueSize: Int) {
            ...
            when {
                insn.opcode == Opcodes.POP -> {
                    ...
                    if (sources.all { !isResultMoved(it) } && sources.any { isTransformablePopOperand(it) }) {
                ...
                insn.opcode == Opcodes.CHECKCAST -> {
                    ...
                    if (sources.all { !isResultMoved(it) } && sources.any { isTransformableCheckcastOperand(it, resultType) }) {
                        transformations[insn] = replaceWithNopTransformation(insn)
                        sources.forEach { propagatePopBackwards(it, inputTop.size) }
                    }
                    else {
        ...
        private fun isResultMoved(insn: AbstractInsnNode) =
                resultMovedInsns.contains(insn)
